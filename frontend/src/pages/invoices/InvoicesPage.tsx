          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th 
                  className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100 select-none"
                  onClick={() => handleSort('administratie__naam')}
                >
                  <div className="flex items-center gap-1">
                    {t('invoices.administratie', 'Administratie')}
                    {filters.ordering?.includes('administratie__naam') && (
                      filters.ordering.startsWith('-') 
                        ? <ChevronDownIcon className="h-3 w-3" />
                        : <ChevronUpIcon className="h-3 w-3" />
                    )}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                <tr>
                  <td colSpan={!isReadOnly ? 11 : 10} className="px-2 py-8 text-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600 mx-auto"></div>
                  </td>
                </tr>
              ) : invoices.length === 0 ? (
                <tr>
                  <td colSpan={!isReadOnly ? 11 : 10} className="px-2 py-8 text-center text-gray-500">
                ...
                invoices.map((invoice) => (
                  <tr key={invoice.id} className={clsx(
                    ...
                    <td className="px-2 py-1.5 max-w-[140px] truncate text-gray-600">
                      {invoice.administratie_naam || <span className="text-gray-300">-</span>}
                    </td>
                    <td className="px-2 py-1.5 whitespace-nowrap">
                      {invoice.week_number ? (
