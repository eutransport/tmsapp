...
const navigation: NavItem[] = [
  ...
  { name: 'nav.factuurwizard', href: '/factuurwizard', icon: SparklesIcon, roles: ['admin', 'gebruiker'], permission: 'use_invoice_wizard' },
  { name: 'nav.factuurwizardBeheer', href: '/factuurwizard/beheer', icon: Cog6ToothIcon, roles: ['admin'], permission: 'manage_invoice_wizard' },
        ...
        >
          ...
          <div className="flex flex-1 items-center justify-end gap-x-2 sm:gap-x-3">
            ...
            <button
              ...
              disabled={verversen}
              className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg touch-target transition-colors touch-manipulation disabled:opacity-60"
              aria-label={t('nav.refreshApp', 'App verversen')}
              title={t('nav.refreshApp', 'App verversen')}
            >
              <ArrowPathIcon
                className={clsx('h-6 w-6 pointer-events-none', verversen && 'animate-spin')}
              />
            </button>
            <NotificationBell />
